		...
		if ((!$_REQUEST['image_title']) || (!$_REQUEST['gallery_name'])) {
			...
			if ($image) {
				...
				$sql = 'INSERT INTO images ( 
					...
					"'.$_REQUEST['image_title'].'", 
					"'.$image.'", 
					"'.$_REQUEST['gallery_name'].'", 
					"'.date("Y-m-d  H:i:s").'", 
					"'.$_REQUEST['weight'].'", 
					"'.$_REQUEST['active'].'"
				)';
				$a = $db->Execute($sql);
				$message = '<div class="success">Your image has been uploaded.</div>';
				$success = true;	
			} else {
				$message = '<div class="error">Oops! You forgot to upload an image.</div>';
				$success = false;
			}
		}
	} else {
...
<?php include ('includes/inc_header.php'); ?>
            ...
            <?php if ($success == true) { ?>
            <div class="actions">
            	<a href="add-image.php">Add another image</a>
            </div>
            <?php } else { ?>
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
                ...
                <dl>
                  ...
                  <dd><input type="text" class="largeTextField" name="image_title" value="<?php echo $_REQUEST['image_title']; ?>" /><span class="tooltip" title="This will display below the large version of the image and will also be used as the value for the ALT tag of the image in the code.">?</span></dd>
                  
                  <dt><label><b>*</b> Upload Image File</label></dt>
                  <dd><input type="file" class="fileUploadField" name="file_name" value="" /><span class="tooltip" title="Image must be optimized for the web at 72 dpi, in RGB color format. Can be JPG, JPEG, PNG, or GIF.">?</span></dd>
                  
                  <dt><label><b>*</b> Gallery</label></dt>
                  <dd>
                    <select name="gallery_name" class="selectMenu">
                    	<option value="">Select a Gallery</option>
                      <option value="child"<?php if ($_REQUEST['gallery_name'] == 'child') { echo ' selected="selected"'; } ?>>Children's Illustrations</option>
                      <option value="oil"<?php if ($_REQUEST['gallery_name'] == 'oil') { echo ' selected="selected"'; } ?>>Oil Paintings</option>
                      <option value="fashion"<?php if ($_REQUEST['gallery_name'] == 'fashion') { echo ' selected="selected"'; } ?>>Fashion</option>
                      <option value="sketch"<?php if ($_REQUEST['gallery_name'] == 'sketches') { echo ' selected="selected"'; } ?>>Sketches</option>
                    </select>
                  </dd>
                  
                  <dt><label>Image Sort Order</label></dt>
                  <dd><input type="text" class="nanoTextField" name="weight" value="<?php echo $_REQUEST['weight']; ?>" /><span class="tooltip" title="Sort is weight based. An image with a sort order of '1' will display in front of an image with a sort order of '2'. You may also use decimals to indicate sort order (1.5, 2.36, etc.)">?</span></dd>
                ...
                <dl>
                  ...
                  <dd>
                  	<?php if ($_REQUEST['active']) { ?>
											<?php if ($_REQUEST['active'] == '1') { ?>
